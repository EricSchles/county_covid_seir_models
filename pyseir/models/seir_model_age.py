...
class SEIRModel:
    ...
    def __init__(self,
                 ...
                 kappa=1,
                 gamma=0.5,
                 contact_preference=np.random.rand(5, 5),
                 ...
                 beds_ICU=100,
                 ventilators=60,
                 mortality_rate=[0.001, 0.0015, 0.003, 0.005, 0.01],
        ...
        self.age_step = self.age_cutoff[1:] - self.age_cutoff[:-1]
    ...
    def _NGM(self, beta):
        P = self.N / self.N.sum()  # initial percentage of susceptibles in each age group
        contact_with_infected = self.contact_preference * P.T

        # transmission matrix with rates of immediate new infections into rows
        # due to transmission from columns
        T_E_E = np.zeros((5, 5))  # from E to E
        T_E_A = contact_with_infected * (1 - self.gamma) * self.beta  # A to E
        T_E_I = contact_with_infected * self.gamma * beta  # I to E
        T_A_E = np.zeros((5, 5))  # E to A
        T_A_A = np.zeros((5, 5))  # A to A
        T_A_I = np.zeros((5, 5))  # I to A
        T_I_E = np.zeros((5, 5))  # E to I
        T_I_A = np.zeros((5, 5))  # A to I
        T_I_I = np.zeros((5, 5))  # I to I
        ...
        aging_rate_in = 1 / self.age_step[:-1]
        aging_rate_out = 1 / self.age_step
        ...
        Z_A_E = np.zeros((5, 5))
        np.fill_diagonal(Z_A_E, self.sigma * (1 - self.gamma))
        Z_A_A = np.diag(-(aging_rate_out + self.delta)) + np.diag(aging_rate_in, k=-1)
        Z_A_I = np.zeros((5, 5))
        # rates of transition out of A (recovery and aging) and into A (aging)
        Z_I_E = np.zeros((5, 5))
        np.fill_diagonal(Z_I_E, self.sigma * self.gamma)
        Z_I_A = np.zeros((5, 5))
        Z_I_I = np.diag(-(aging_rate_out + self.delta)) + np.diag(aging_rate_in[1:], k=-1)
    ...
    def _estimate_beta(self, expected_R0):
        ...
        for beta in betas:
            R0 = self.NGM(beta)
            R0s.append(R0)
        f = scipy.interpolate.interp1d(R0s, beta)
        beta = f(expected_R0)
        return beta
    ...
    def _time_step(self, y, t):
        ...
        frac_infected = (self.kappa * I + A) / N
        contacts_S_and_I = S[:, np.newaxis].dot(frac_infected[:, np.newaxis].T)
        effective_contacts = (contacts_S_and_I * self.contact_preference).sum(axis=1)
    ...
    def run(self):
        ...
        self.results = {
            ...
            # self.mortality_rate_no_general_beds / self.hospitalization_length_of_stay_general,
